...
做 PPT 的第一步就是确定演讲的主题了，我选择了一个比较简单和有趣的主题，就是如何实现下面的效果：当你的鼠标移动上去的时候，会出现类似蛇一样残影的效果。这里可以[在线体验](https://www.openprocessing.org/sketch/862903)。
...
目前 Gossip 只是支持四种元素组成：`容器`、`文字`、`图片`、和 `画布`。对于每一个元素，在**预览面板**中：
