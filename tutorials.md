...
目前 Gossip 只是支持四种元素组成：`容器`、`文字`、`图片`、和`画布`。对于每一个元素，**鼠标点击一下会选择该元素，鼠标点击被选择的元素会进入编辑模式，当鼠标移开的该元素时候，会退出编辑模式**。
...
但是现在图片和文字是竖着排列的，在**结构面板**找到包含这段文字和图片的**容器**，将其**排列方式改**成**水平**即可。这之后修改一下**比例**，然后调整一下文字的**对齐方式**。
...
接下来分别将变量拖进“监听事件”和“移动数组”的标题的**字体颜色**，这时发现它们的标题颜色也都从**黑色**变成了**红色**。
...
现在将“获得画布”中的**文字**删除，在第二个**竖直容器**中插入一张图片，然后将**想法面板**中第一张代码片段图片直接拖入该图片，这之后调整一下字体的**对齐方式**。对剩下两张幻灯片重复上诉操作，注意拖入不同的代码片段的图片。
...
现在鼠标在画布上移动，就会出现在另外一个网页相同的效果！这样你在给别人展示一些很酷的东西的时候，就不需要重新打开一个网页,一切都是一体的。
...
**词云模式也是 Gossip 相对与普通幻灯片软件与众不同的地方**，提供一种一眼了解该 PPT 所有关键信息的模式，在演示过程中按下 `backspace` 进入**词云模式** 。
...
这样用户可以一言看出演讲者想表达的故事最重要的部分，不需要一张一张的看。

同时，在演讲结束后往往会有一个提问环节，会出现提问者让演讲者不断滚动缩略图来找到想要提问的幻灯片。但是 Gossip 就不存在这个问题，在词云模式下，提问者可以直接找到自己有问题的幻灯片，然后演讲者点击该幻灯片，该幻灯片会自动放大。效果如下。

![8jsIn1.gif](https://s1.ax1x.com/2020/03/25/8jsIn1.gif)

当让这个案例只是一个比较简单的案例，每一页幻灯片都比较简单。稍微复杂的案例可以看 Gossip 的介绍 PPT。
